...
//mongodb+srv://masao:muromi2024@cluster0.wdz33.mongodb.net/?retryWrites=true&w=majority&appName=Cluster0
...
app.delete("/item/delete/:id",auth ,async(req,res)=>{
    try{
        ...
        const singleItem = await ItemModel.findById(req.params.id)
        if(singleItem.email === req.body.email){
            await ItemModel.deleteOneOne({_id: req.params.id})
...
app.post("/user/login",async(req,res)=>{
    try{
        ...
        if(savedUserData){
            if(req.body.password === savedUserData){
                const payload = {
